...
public class ClosestPair {
    ...
    private float closest (Point2D[] pointsByX, Point2D[] pointsByY, Point2D[] aux, int lo, int hi) {
        ...
        float delta1 = closest(pointsByX, pointsByY, aux, lo, mid);
        float delta2 = closest(pointsByX, pointsByY, aux, mid+1, hi);
        float delta = Math.min(delta1, delta2);
        ...
        for (int i = 0; i < M; i++) {
            ...
            for (int j = i+1; (j < M) && (aux[j].y() - aux[i].y() < delta); j++) {
                float distance = aux[i].distanceTo(aux[j]);
                if (distance < delta) {
                    delta = distance;
     ...
     *         <tt>Float.POSITIVE_INFINITY</tt> if no such pair of points
     *         exist (because there are fewer than 2 points)
     */
    public float distance() {return bestDistance;}
