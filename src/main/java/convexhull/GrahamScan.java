...
class GrahamScan implements Iterable<Point2D> {
    final private Stack<Point2D> hull = new Stack<>();

    public int size () {return hull.size();}

    @Override
    public Iterator<Point2D> iterator() {return hull.iterator();}
    ...
    public boolean isWithinHull (Point2D p) {
        Point2D prev = null;
        for (Point2D curr : hull()) {
            if (prev!=null)
                if (Point2D.ccw(prev, curr, p)<0)
                    return false;
            prev = curr;
        }
        return true;
