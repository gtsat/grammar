...
final public class MinMaxComparator<NodeT> implements Comparator<NodeT> {
    ...
    public MinMaxComparator (Group<NodeT> group,Graph<NodeT> network,ShortestPath<NodeT> shp) {
        grouping = group;
        graph = network;
        sp = shp;
    }

    public MinMaxComparator (Group<NodeT> group,Graph<NodeT> network) {
        grouping = group;
        graph = network;
        sp = null;
    }
    ...
    public int compare (NodeT x, NodeT y) {
        ...
        double xmax=computeScore(x), ymax=computeScore(y);
        double balance = xmax-ymax;
    ...
    public double computeScore (NodeT u) {
        if (cache.containsKey(u)) return cache.get(u);
            ...
            for (Group<NodeT> subgroup : grouping.getSubgroups()) {
                double dist = subgroup.getLambdas().get(subgroup)
