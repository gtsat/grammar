...
final public class LocationComparatorMinMax<NodeT> implements Comparator<NodeT> {
    ...
    final private float[] individualrelaxation;
    ...
    public LocationComparatorMinMax (Group<NodeT> group, Graph<NodeT> network,ShortestPath<NodeT> shp,boolean bound) {
        ...
        int i=0;
        float summedproducts = 0.0f;
        individualrelaxation = new float [group.getSubgroups().size()];
        for (Group<NodeT> over : group.getSubgroups()) {
            float product=1.0f;
            for (Group<NodeT> under : group.getSubgroups())
                product *= over.getLambdas().get(under);
            individualrelaxation [i++] = product;
            summedproducts += product;
        }
        relaxation = summedproducts;
    ...
    public boolean isRelevant (NodeT u) {
        ...
        for (Group<NodeT> group : grouping.getSubgroups()) {
            float groupcost = group.getLambdas().get(group) 
                      * (sp==null?graph.getPathCost(group.getTarget(),u):sp.getPathCost(group.getTarget(),u)) 
                      + individualrelaxation[i++] * remainingdistance ;
            float individualcost = group.getLambdas().get(group) 
                      * (sp==null?graph.getPathCost(group.getTarget(),target):sp.getPathCost(group.getTarget(),target));
            if (individualcost>groupcost) return false;
        }
        return true;
    }
    ...
    public float computeScore (NodeT u) {
        if (cache.containsKey(u)) {
            ...
            float score = 0.0f;
            for (Group<NodeT> group : grouping.getSubgroups()) {
                float newscore = group.getLambdas().get(group) 
                        * (sp==null?graph.getPathCost(group.getTarget(),u):sp.getPathCost(group.getTarget(),u));
