...
abstract class DivSetComparatorOrdered<NodeT> implements Comparator<ArrayList<NodeT>> {
    ...
    public DivSetComparatorOrdered (NodeT q,Graph<NodeT> graph,ContentSimilarity<NodeT> context,float l,float a,float b) {
...
final class SumDivSetComparatorOrdered<NodeT> extends DivSetComparatorOrdered<NodeT> {
    public SumDivSetComparatorOrdered(NodeT q,Graph<NodeT>graph,ContentSimilarity<NodeT>context,float l,float a,float b) {super(q,graph,context,l,a,b);}
    @Override public float computeScore (ArrayList<NodeT> set) {
        ...
        if (cached!=null) return cached;
            ...
            for (int i=0;i<set.size();++i) {
                ...
                qfactor += alpha*graph.getPathCost(q,s) + 
                           (1-alpha)*graph.numberNodes()*(1-context.similarity(q,s));
                for (int j=i+1; j<set.size(); ++j) {
                    NodeT recipr = set.get(j);
                    Sfactor += beta*graph.getPathCost(s,recipr) + (1-beta)*graph.numberNodes()*(1-context.similarity(s,recipr));
                }
            }
            float score = (lambda*qfactor - 2*(1-lambda)*Sfactor/(set.size()-1))/set.size();
            cache.put (set,score);
            return score;
        }
    }
...
final class MaxDivSetComparatorOrdered<NodeT> extends DivSetComparatorOrdered<NodeT> {
    public MaxDivSetComparatorOrdered(NodeT q,Graph<NodeT>graph,ContentSimilarity<NodeT>context,float l,float a,float b) {super(q,graph,context,l,a,b);}
    @Override public float computeScore (ArrayList<NodeT> set) {
        Float cached = cache.get(set);
        if (cached!=null) return cached;
        else{
            ...
            for (int i=0;i<set.size();++i) {
            NodeT s = set.get(i);
                float qdist = alpha*graph.getPathCost(q,s) + 
                               (1-alpha)*graph.numberNodes()*(1-context.similarity(q,s));
