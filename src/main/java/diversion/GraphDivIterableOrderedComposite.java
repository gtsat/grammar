...
final class GraphDivIterableOrderedComposite<NodeT> implements Iterable<NodeT> {
    ...
    public GraphDivIterableOrderedComposite (NodeT q, Collection<NodeT> S, Collection<NodeT> R, 
                             Graph<NodeT> graph,
...
*/
        ...
        @Override public NodeT next () {
            ...
            while (completeScoresHeap.isEmpty() 
                ...
                if (minCostExpansionHeap!=null) {
                    ...
                    while (!distanceheapFrom.isEmpty() && (scoreheap.isEmpty() || gamma * distanceheapFrom.min().weight < scoreheap.min().weight)) { //cmp.scores.get(scoreheap.min().to))) {
                        Edge<NodeT> top = distanceheapFrom.delMin();

                        if (logMemory) reservedBytesProcessing -= 20;

                        float newscore = gamma * top.weight;
    ...
    public static void main (String[] args) {
        ...
        int i=0;
        for (Long mu : new GraphDivIterableOrderedComposite<>(q,S,graph.getNodes(),graph,context,useMinSum,0.5f,0.0f,1.0f))
            if (++i>=k) break;
    }
}
