...
final public class EvaluateKappa {
    ...
    public static void main (String[] args) throws IOException {
        ...
        for (String network : networks) {
            ...
            ArrayList<ArrayList<Long>> trajectories = loadTrajectories ("../queries/trajectories/"+network+"/"+distances[Index],100);
            ...
            for (int kappa : kappas) {
                for (int i=0; i<trajectories.size();) {
                    ...
                    Collection<Edge<Long>> result = ClosestPairs.computeMultiThreaded (gp,R,S,kappa,Float.MAX_VALUE,parallelism[Index]);
                    long endTime = System.nanoTime();
                    duration += (endTime - startTime)*Math.pow(10,-9); // normalized in seconds
                }
                System.out.println("["+kappa+" "+duration/(trajectories.size()>>1)+"]");
            }   
        }
    }
