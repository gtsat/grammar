...
final public class EvaluatePartitions {
    final static private int[] kappas = {20,40,60,80,100,120,140,160};
    final static private int[] parallelism = {2,4,16,8,10,12,14,16};
    ...
    //final static private String[] networks = {"SF"};
    ...
    public static void main (String[] args) throws IOException {
        ...
        for (String network : networks) {
            ...
            int i=0;
            Object[] gps = new Object [partitions.length];
            for (int p : partitions) {
                GraphPartition<Long> gp = readPartition?
                    new GraphPartition<>(graph,"../data/lifeifei/A1.0/"+network+p+"/")
            ...
            i=0;
            for (Object o : gps) {
                GraphPartition<Long> gp = (GraphPartition<Long>) o;
                long startTime = System.nanoTime();
                ArrayDeque<Edge<Long>> result = ClosestPairs.computeMultiThreaded (gp,R,S,kappas[Index],Float.MAX_VALUE,parallelism[Index]);
                long endTime = System.nanoTime();
                float duration = (endTime - startTime) * (float) Math.pow(10,-9);
                //System.out.println("["+p+" "+duration+"]");
                System.out.println("["+partitions[i++]+" "+duration+"]");
            }
        }
    }
