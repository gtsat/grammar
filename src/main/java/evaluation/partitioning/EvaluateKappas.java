...
final public class EvaluateKappas {
    ...
    public static void main (String[] args) throws IOException {
        ...
        for (String network : networks) {
            ...
            for (int kappa : kappas) {
                long startTime = System.nanoTime();
                ArrayDeque<Edge<Long>> result = ClosestPairs.computeMultiThreaded (gp,R,S,kappa,Float.MAX_VALUE,parallelism[Index]);
                long endTime = System.nanoTime();
                float duration = (endTime - startTime) * (float) Math.pow(10,-9);
                System.out.println("["+kappa+" "+duration+"]");
            }
        }
    }
