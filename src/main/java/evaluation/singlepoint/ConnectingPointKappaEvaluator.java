...
final public class ConnectingPointKappaEvaluator {
    final static private int[] travelers = {2,4,8,16};
    final static private float[] coverages = {.05f,.10f,.20f,.40f,.80f};
    final static private float[] selflambdas = {.3f,.5f,.7f,.9f};
    final static private int[] kappas = {1,2,4,8,16};
    ...
    public static void main (String[] args) throws IOException {
        ...
        for (String network : networks) {
            System.out.println ("** Evaluation of result size scalability using the "+network
            ...
            int n = travelers [1];
            float area = coverages [3];
            float selflambda = selflambdas [2];
            ...
            ArrayDeque<Long[]> queries = loadQueries("../queries/demo/"+network+"/QueryN"+n+"A"+(int)(area*100)+".txt");
...
*/
                ...
                System.out.print ("["+k+"\t"+bnbTime/Math.pow(10,9)/Qid+"\t"+ceTime/Math.pow(10,9)/Qid+"\t"+relTime/Math.pow(10,9)/Qid+"\t"+postTime/Math.pow(10,9)/Qid+"\t"
