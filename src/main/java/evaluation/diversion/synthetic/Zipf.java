...
final class Zipf {
    ...
    private void init (int r) {
        ...
        // Test init:
        if(debug){
            for(int i = 0; i<tester.length; i++) tester[i]=0;
            for(int i=0; i<size; i++)
                System.out.println("Index: " + i + ", Dist-array values (" 
                                    + pDist[i] + ", " + numbers[i] + ")");
        }
    }
    ...
    public void printTestArray () {
        ...
        }else System.out.println("Debuging not enabled...");
    }

    private double computeHarmonic (int n) {
        double sum=0;
        for (int k=1;k<=n;k++) sum += (1.0/(double)k);
        return sum;
    }

    private double density (double d, double h) {return 1.0/(d*h);}

    public static void main (String[] args) {

        int range=1000;
        Zipf zipf = new Zipf (range);

        double factor = 1.0/range;
        for (int i=0;i<100;++i) {
            System.out.println (zipf.nextZipf()*factor+" ");
        }
    }
}
